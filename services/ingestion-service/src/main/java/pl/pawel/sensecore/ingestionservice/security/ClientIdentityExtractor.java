...
//    private static final String HEADER_DN = "X-SSL-Client-DN";
    ...
    public ClientIdentity extract(HttpServletRequest request) {
        ...
        if (cert == null || cert.isBlank()) {
            log.warn("Client certificate header is missing");
        ...
        if (fingerprint.isBlank()) {
            log.warn("Fingerprint extraction failed");
    ...
    private String extractIp(HttpServletRequest request) {
        ...
        if (xff != null && !xff.isBlank()) {
            ...
            log.debug("Extracted client ip from X-Forwarded-For");
    ...
    private String header(HttpServletRequest request, String name) {
        ...
        log.debug("Reading header: {} present={}", name, value != null && !value.isBlank());
        return value;
    }
